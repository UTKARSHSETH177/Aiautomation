...
from project_paths import DB_PATH, RAW_DATA_DIR, ensure_runtime_dir
...
def load_source2(path):
    ...
    email sits in the wrong column). Returns (csv_line_no, row_dict) pairs so
    downstream logs reference REAL file lines even after rows are dropped."""
    ...
    for i, r in enumerate(rows, start=2):  # 1-based incl. header
        ...
        if "@" not in r[0]:
            ...
            if email_pos is not None:
                ...
                continue
        fixed.append((i, dict(zip(header, r))))
    return fixed


def load_source3(path):
    """Returns (csv_line_no, row_dict) pairs — see load_source2."""
    header, *rows = read_csv(path)
    out = []
    for i, r in enumerate(rows, start=2):
        if r == header:
            log_issue("source3", f"line {i}", "repeated header row inside data",
                      "dropped")
            continue
        out.append((i, dict(zip(header, r))))
    return out
...
class PersonIndex:
    ...
    def _merge_into(self, pid, name, email, phone, city, source, how):
        ...
        p["notes"].append(how)
        if name and p["name"] and not names_match(name, p["name"]):
            log_issue(source, name,
                      f"hard-identifier match to person {pid} but names "
                      f"disagree ('{name}' vs '{p['name']}')",
                      "merged on identifier; name kept, disagreement flagged")
...
def ingest(s1_path, s2_path, s3_path, db_path=DB_PATH):
    """Build a fresh database from the three source files."""
    ISSUES.clear()
    ...
    for i, r in enumerate(load_source1(s1_path), start=2):
        ...
        if (r.get("City") or "").strip().lower() == "delhi ncr":
            log_issue("source1", f"line {i} ({name})",
                      "region 'Delhi NCR' recorded as city",
                      "mapped to canonical city 'Delhi'")
        ...
        if date and date > datetime.now().date().isoformat():
            log_issue("source1", f"line {i} ({name})",
                      f"applied_date {date} is in the future",
                      "stored as-is, flagged for review")
    ...
    seen_gigs = set()
    for i, r in load_source2(s2_path):
        key = tuple((r.get(k) or "").strip().lower()
                    for k in ("email_id", "rate", "skill_tags"))
        if key in seen_gigs:
            log_issue("source2", f"line {i} ({r.get('worker_name')})",
                      "exact duplicate gig profile (identical email/rate/"
                      "skills) — the column-shifted row, once repaired, "
                      "duplicated an earlier row", "dropped; one profile kept")
            continue
        seen_gigs.add(key)
        name = norm_name(r.get("worker_name"))
        email = norm_email(r.get("email_id"))
        city = norm_city(r.get("location"))
        if (r.get("email_id") or "").strip() and email is None:
            log_issue("source2", f"line {i} ({name})",
                      f"unparseable email '{r.get('email_id')}'", "stored NULL")
        if (r.get("location") or "").strip() and city is None:
            log_issue("source2", f"line {i} ({name})",
                      f"unmapped location '{r.get('location')}'", "stored NULL")
    ...
    for i, r in load_source3(s3_path):
        ...
        if (r.get("City") or "").strip().lower() == "delhi ncr":
            log_issue("source3", f"line {i} ({name})",
                      "region 'Delhi NCR' recorded as city",
                      "mapped to canonical city 'Delhi'")
        if (r.get("City") or "").strip() and city is None:
            log_issue("source3", f"line {i} ({name})",
                      f"unmapped city '{r.get('City')}'", "stored NULL")
    ...
    if webhook_url:
        ...
        try:  # optional fire-and-forget integration
            urllib.request.urlopen(
                urllib.request.Request(webhook_url, method="POST"), timeout=5
            )
            print("triggered n8n skill-tagger webhook")
        except Exception as exc:
            print(f"n8n webhook not reachable ({exc}) — trigger manually")


if __name__ == "__main__":
    args = sys.argv[1:] or [
        RAW_DATA_DIR / "source1_naukri_applicants.csv",
        RAW_DATA_DIR / "source2_gig_workers.csv",
        RAW_DATA_DIR / "source3_cbnexus_contacts.csv",
    ]
    ingest(*args)
